...
class SparseScoreRoIHead(CascadeRoIHead):
    ...
    def __init__(self,
        ...
        super(SparseScoreRoIHead, self).__init__(
            num_stages,
            stage_loss_weights,
    ...
    def _mask_forward(self, stage, x, rois, attn_feats):
        ...
        # mask_pred, x_feat = mask_head(mask_feats, attn_feats)

        mask_results = dict(mask_pred=mask_pred)
        # mask_results = dict(mask_pred=mask_pred, mask_feats=x_feat)
        return mask_results
    ...
    def _mask_forward_train(self, stage, x, attn_feats, sampling_results,
        ...
        # mask_results['mask_pred'] in [sum_pos_of_all_images, 1, 28, 28]
        # pos_mask_pred in [sum_pos_of_all_images, 28, 28]
        # attn_feats in [sum_pos_of_all_images, 256]
        # mask_results['mask_feats'] in [sum_pos_of_all_images, 256, 14, 14]
        # pos_mask_pred = mask_results['mask_pred'][
        #     range(mask_results['mask_pred'].size(0)), pos_labels]
        # mask_iou_pred = self.mask_iou_head[stage](mask_results['mask_feats'], pos_mask_pred)
        # pos_mask_iou_pred = mask_iou_pred[range(mask_iou_pred.size(0)),
                                        #   pos_labels]

        # mask_iou_targets = self.mask_iou_head[stage].get_targets(
        #     sampling_results, gt_masks, pos_mask_pred,
        #     mask_targets, rcnn_train_cfg)
        # loss_mask_iou = self.mask_iou_head[stage].loss(pos_mask_iou_pred,
                                                # mask_iou_targets)
        # mask_results.update(loss_mask_iou)
        return mask_results
    ...
    def forward_train(self,
        ...
        Args:
            ...
            proposals (Tensor): Decoded proposal bboxes, has shape
                (batch_size, num_proposals, 4), in [x1, y1, x2, y2] format
        ...
        for stage in range(self.num_stages):
            ...
            proposal_list = bbox_results['detach_proposal_list'] # xyxy format
            ...
            if self.with_mask:
                ...
                # single_stage_loss['loss_mask_iou'] = mask_results['loss_mask_iou']
    ...
    def simple_test(self,
        ...
        if self.with_mask:
            ...
            # mask_iou_score = self.mask_iou_head[stage](mask_results['mask_feats'], mask_results['mask_pred'][:, 0])
            mask_results['mask_pred'] = mask_results['mask_pred'].reshape(
                num_imgs, -1, *mask_results['mask_pred'].size()[1:])

        # cls_score = torch.sqrt(cls_score * mask_iou_score.reshape(num_imgs, -1, 1))
        ...
        for img_id in range(num_imgs):
            ...
            labels_per_img = topk_indices % num_classes # labels_per_img = 0
